...
def harrison_ford
  ...
  # Actor.find_by(name: "Harrison Ford").movies.where("castings.ord != 1")
...
def directed_by_one_of(them)
  ...
  # Movie.select(:id, :title).joins(:director).where(name: them)
  Movie.select(:id,:title).joins(:director).where(actors: {name: them})
  # Movie.select(:id, :title).where(director_id: Actor.where(name: them))
end
